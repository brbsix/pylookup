...
class Rebuild:
    """Rebuild the script's message list."""
    ...
    def check(self):
        ...
        self.data = subprocess.check_output(command, executable='bash', shell=True, stderr=subprocess.PIPE).decode().strip()

        if not self.data:
            fatal('Unable to acquire messages necessary to rebuild')

        exec('NEW_MSGLIST = {' + self.data + '}', globals())
    ...
    def exception(self):
        ...
        if os.path.isfile(__file__ + '.bak'):
            if os.path.isfile(__file__):
                remove(__file__)
            move(__file__ + '.bak', __file__)

    def finish(self):
        # set permissions using backup as a reference
        chmod(__file__, __file__ + '.bak')
...
def fatal(*args):
    """Display error message via stderr before exiting."""
    error(*args)
    sys.exit(1)
